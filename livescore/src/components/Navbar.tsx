  ...
  function ThemeToggle({ className = "" }: { className?: string }) {
    return (
      <button
        ...
        className={`flex items-center justify-center w-9 h-9 rounded-lg border border-border hover:bg-surface-light transition-colors text-text-secondary hover:text-text ${className}`}
      >
        {theme === "dark" ? (
            ...
            <div className="border-t border-border px-4 py-3 space-y-2">
              ...
              {socialLinks.tiktok && (
                <a
                  ...
                  rel="noopener noreferrer"
                  onClick={() => setMobileOpen(false)}
                  className="flex items-center justify-center gap-2 w-full px-3 py-2.5 rounded-xl bg-[#010101] text-white hover:bg-[#333333] transition-all"
                >
                  <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
