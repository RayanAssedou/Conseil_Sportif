    ...
    <div className="max-w-7xl mx-auto px-4 py-6 space-y-8">
      ...
      <div className="relative rounded-2xl overflow-hidden">
        {hero.background_type === "image" && hero.background_value ? (
          ...
          <div
            className="bg-white dark:bg-[#111111] min-h-[180px] md:min-h-[260px]"
            style={heroStyle}
          />
        )}

        {/* Join חמ"ל WhatsApp community */}
        <a
          href="https://chat.whatsapp.com/EiEdeBPSi3R2YTBvFyIn9I"
          target="_blank"
          rel="noopener noreferrer"
          className="absolute bottom-3 left-1/2 -translate-x-1/2 flex items-center gap-2 rounded-full bg-[#25D366] px-5 py-2.5 text-sm sm:text-base font-bold text-white shadow-lg shadow-black/25 ring-1 ring-white/25 transition-all hover:bg-[#1ebe5b] hover:scale-[1.03] active:scale-[0.98] whitespace-nowrap max-w-[calc(100%-1.5rem)]"
        >
          ...
          <span className="truncate">{t("home.joinHamalCommunity")}</span>
        </a>
      </div>
      ...
      <a
        ...
        className="relative overflow-hidden flex items-center gap-3 rounded-2xl border border-amber-400/50 bg-gradient-to-br from-amber-500 to-amber-600 p-4 text-white shadow-[0_0_24px_rgba(245,158,11,0.25)] transition-all hover:scale-[1.01] active:scale-[0.99]"
      >
        <div className="absolute -left-8 -top-8 w-28 h-28 rounded-full bg-white/10" />
        <div className="relative w-10 h-10 rounded-xl bg-white/15 flex items-center justify-center flex-shrink-0">
        ...
        <span className="relative flex-1 text-base sm:text-lg font-bold">{t("home.vipSingle")}</span>
        <svg className="relative w-6 h-6 text-white flex-shrink-0" fill="currentColor" viewBox="0 0 24 24">
          <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
        </svg>
      </a>
        ...
        >
              ...
              <a href={tiktokLink || "https://www.tiktok.com/@niv_winner_tips?_r=1&_t=ZS-958O1XrBfQC"} target="_blank" rel="noopener noreferrer" className="flex-shrink-0 bg-gradient-to-br from-[#010101] to-[#333333] rounded-xl p-3.5 text-white" style={{ width: "80vw", scrollSnapAlign: "start" }}>
                <div className="flex items-center gap-2.5">
                  ...
                  <div className="min-w-0 flex-1">
                    <h3 className="text-sm font-bold">{t("home.tiktokTitle")}</h3>
                    <p className="text-white/70 text-[11px] leading-tight">{t("home.tiktokDesc")}</p>
                  </div>
                  <span className="flex-shrink-0 inline-flex items-center gap-1 px-2.5 py-1.5 bg-white/20 rounded-lg text-[11px] font-semibold">
                    {t("home.followTiktok")}
                    <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M13.5 6H5.25A2.25 2.25 0 003 8.25v10.5A2.25 2.25 0 005.25 21h10.5A2.25 2.25 0 0018 18.75V10.5m-10.5 6L21 3m0 0h-5.25M21 3v5.25" /></svg>
      ...
      <section className="hidden md:grid gap-4 grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 auto-rows-fr">
        ...
        >
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              <h3 className="text-lg font-bold">{t("home.tiktokTitle")}</h3>
