...
export default function SettingsPage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      .then(([sData, tData, wData, iData, fpData, gtData, wvData]) => {
        setSection(sData || {});
        setTelegram(tData || {});
        setWhatsapp(wData || {});
        setInstagram(iData || {});
        ...
        <form onSubmit={(e) => { e.preventDefault(); saveSocial(whatsappVip, "whatsapp_vip", setSavingWhatsappVip, setSavedWhatsappVip); }}>
          <div className="bg-white rounded-xl border border-slate-200 p-6 space-y-5">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-base font-semibold text-slate-900">WhatsApp VIP Community</h2>
                <p className="text-sm text-slate-500">Set the exclusive VIP WhatsApp group invite link</p>
              </div>
              <span className="ml-auto px-2.5 py-1 text-xs font-bold bg-gradient-to-r from-amber-100 to-amber-200 text-amber-700 rounded-full uppercase tracking-wider">VIP</span>
            </div>

            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1.5">VIP WhatsApp Link</label>
              <div className="flex gap-2">
                <div className="flex items-center px-3 bg-gradient-to-b from-amber-50 to-amber-100 border border-r-0 border-slate-200 rounded-l-lg text-sm text-amber-600">
                  <svg className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor">
                    <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
                  </svg>
                </div>
                <input
                  ...
                  className="flex-1 px-4 py-3 border border-slate-200 rounded-r-lg focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 outline-none transition"
                />
              </div>
              <p className="text-xs text-slate-400 mt-1.5">Paste your VIP WhatsApp group invite link (e.g. https://chat.whatsapp.com/xxxxx)</p>
            </div>
            ...
            <div className="flex items-center gap-3 pt-2">
              <button
                ...
                className="flex items-center gap-2 px-6 py-2.5 bg-gradient-to-r from-amber-500 to-amber-600 text-white text-sm font-semibold rounded-lg hover:from-amber-600 hover:to-amber-700 transition-all shadow-sm disabled:opacity-50"
              >
                {savingWhatsappVip ? "Saving..." : "Save VIP Link"}
              </button>
              {savedWhatsappVip && (
                <span className="flex items-center gap-1.5 text-sm text-green-600 font-medium">
                  <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
