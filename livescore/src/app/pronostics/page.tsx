    ...
    <div className="max-w-7xl mx-auto px-4 py-6">
      <div className="mb-4">
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-gradient-to-r from-violet-500/10 to-blue-500/10 border border-violet-500/20">
            ...
            <span className="text-[11px] font-bold bg-gradient-to-r from-violet-600 to-blue-600 bg-clip-text text-transparent">{t("ai.badge")}</span>
          </span>
        </div>
        <p className="text-sm text-text-muted mt-1">
          {t("predictions.subtitle")} · <span className="text-violet-500 font-medium">{t("ai.shortDesc")}</span>
        </p>
      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 blur-[6px] pointer-events-none select-none" aria-hidden="true">
            {grouped.slice(0, 3).map(([league, preds]) => (
              <div key={league} className="flex flex-col">
                ...
                <div className="bg-card border-x border-b border-border rounded-b-xl overflow-hidden divide-y divide-border/50">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-white/60 backdrop-blur-sm rounded-2xl">
            ...
            <div className="flex gap-3">
              ...
              <Link
                ...
                className="px-6 py-2.5 bg-card text-primary font-semibold text-sm rounded-lg border-2 border-primary hover:bg-primary/5 transition-colors"
              >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {grouped.map(([league, preds]) => (
            <div key={league} className="animate-slide-up flex flex-col">
              ...
              {/* Predictions */}
              <div className="bg-card border-x border-b border-border rounded-b-xl overflow-hidden divide-y divide-border/50 flex-1">
                {preds.map((pred) => (
                  <Link
                    key={pred.fixture_id}
