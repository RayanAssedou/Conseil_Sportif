...
const DEFAULT_WHATSAPP = "https://wa.me/972504593270";
...
function SignInContent() {
  ...
  useEffect(() => {
    fetch("/api/content/section?key=whatsapp")
      .then((r) => r.json())
      .then((d) => {
        const url = d?.view_all_link;
        if (typeof url === "string" && url.startsWith("http")) setWhatsappLink(url);
      })
      .catch(() => {});
  }, []);
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4 bg-surface">
      <div className="w-full max-w-sm">
        ...
        {mode === "signup" && (
          <div className="mt-4 bg-card border border-border rounded-2xl p-4 text-center shadow-sm">
            <p className="text-sm font-medium text-text mb-3">{t("auth.signupHelpTitle")}</p>
            <a
              href={whatsappLink}
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center justify-center gap-2 w-full px-4 py-2.5 rounded-xl bg-[#25D366] text-white text-sm font-semibold hover:bg-[#1da851] transition-colors shadow-sm"
            >
              ...
              <span>{t("auth.signupHelpButton")}</span>
            </a>
            <p className="text-xs text-text-muted mt-2">{t("auth.signupHelpNote")}</p>
          </div>
        )}
      </div>
    </div>
